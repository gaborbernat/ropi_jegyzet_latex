...
cimkét vezessünk: az egyik azt mutatja, hogy a pont elért e, és ha igen mely
...
Az átvizsglálási sorrend lesz a topologikus sorrend, ha $s$ nem tagja az eredeti
gráfnak töröljük azt. Ha megfelelő adatstruktúrát használunk mindez az élszámmal
arányos időt vesz igénybe. Példa feladatt:
...
A bizonyitáshoz, tegyük fel, hogy az SPT nem ad optimális megoldást. Legyen
...
felcserélés végre hajtéséval a $\sum C_j$ csökken, de ez ellentmondás a
feltevésünkenk, tehát az SPT optimális.

\subsection{ \texorpdfstring {$ P_2||C_max$} {P2||Cmax} }

Ez egy NP--nehéz feladatt mivel visszavezethető a partició problémára. Legyen
$a_1, a_2, \cdots, a_n \in \mathbb{Z}^*$. A kérdés, hogy létezzik e $I \subseteq
\{1,2, \cdots, n \}$ amire $\sum_{i \in I} a_i = \sum_{i \not \in I} a_i$. Ez
meg a részösszeg speciális esete. Vegyük most a megmunkálási időket $\{p_1$, $p_2$,
$\cdots$, $p_n\}$, ekkor, ha OPT$=\frac{\sum P_i}{2} \Leftrightarrow$ létezzik $p$
számsorozatnak jó partició. 
...
\begin{description}
  ...
  $P_{i=\overline{1,n}}$ feladatt. Rakjuk fel ezeket sorrendben a gépekre, ha
  valamely gép végez megkapja a listában a következő munkát.
\end{description}

Ez egy elég primitiv módszer hisszen gyakorlatilag csak arra ügyelünk, hogy a
...
pillantban kezdödik el és tart $P_k$ idő intervallumot ($t+p_k=C_{\mbox{max}}$).
...
Látható a bizonyitásból, hogy a $P_k$ mérete döntően befolyásolja az ütemezés
hosszát, így ha ez kicsi javíthatunk az approximációs faktoron. 
\begin{description}
  \item[Longest Procesisng Time -- LPT ] Rakjuk ezért a munkákat csökenő
  sorrendbe, és e új lista szerint ütemezünk (a legrövidebb marad a végire).
\end{description}
 
Az LPT ütemezés appróximációs faktora $\left( \dfrac{4}{3} - \dfrac{1}{3m}
\right)$. E jó appróximációs közelités annak köszönhető, hogy a gépek egyforma
...
munka elkezdhető. Ez egy $2-\frac{1}{m}$ appróximáció. Ezen való javitás kulcsa,
hogy hiába fejezzük be az utolsó feladathoz közeli feladatott mert a távoliak is
be kell érjék ezeke ( feltéve, hogy ezek függenek egymástól). 

Legyen $D$ írányitótt gráf a precedencia mátrixa a munkáknak. Ha bármely
írányitott úton összeadjuk az utakat alkotó pontok megfelelő megmunkálási
időeket, akkor alsó becslést kapunk egy optimális megmunkálási időre.

Tehát egy pont szintje legyen a pontból kiinduló írányitott utak  mentén vett
megmunkálási idők összegének maximuma. Ezt megkaphatjuk, ha topologikus
sorrendbe állitjuk a munkákat, majd felfelé lépve számoljuk a szint értékeket.
Rendezzük a munkákat a szintjük szerint csökenő sorrendbe, és e lista szerint
ütemezünk. 

Jobb közelitést ez sem add, visszont ha az élszámunk magas jó eredményt szokott
...
appróximáció mert az optimális és kapott arány $\frac{10}{6}=\frac{5}{2} \geq
\frac{3}{2}$.
...
Ebben az alakban a feladatt NP--nehéz. Ha $m=2$ gép ál rendelkezésünkre a
\emph{Coffman-Graham} algoritmus optimális megoldást ad. $D$ precedencia gráfon
végezzük el a tranzitiv redukciót:
...
Majd bontsuk csoportokra a csúcsókat: az első szinten azon csoportban találhatók
akik kifoka $0$, a másodikon akik kifoka $0$, ha elhagyjuk az első szinten levő
elemeket és így tovább. Majd az első szinten számozuk meg a pontokat $1$--től
...
\begin{itemize}
  ...
  csökenő sorrendben. E számozásokat lexikografikusan növekvő sorrendbe
  rendezésse adja a csoporton belüli sorrendet.
  \item második körben a csoportbeli pontokat számozzuk át, folytatva a korábbi
  csoport számozásától a kapott csoportbeli sorrendel.
\end{itemize}
 
A kapott számozás szerint csökenő sorrendbe rakjuk fel a gépekre a munkákat mint
...
olyan írányitott gyökeres fa, aminek az élei a gyökér felé vannak írányitva.},
akkor a szint szerinti ütemezés (gyökértől levő úthosz csökenő sorrendben)
